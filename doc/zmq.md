...
The Mundoteam Core daemon can be configured to act as a trusted "border
router", implementing the mundoteam wire protocol and relay, making
...
The ZeroMQ feature in Mundoteam Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building mundoteamd:
...
For instance:
    ...
    $ mundoteamd -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubhashtx=tcp://192.168.1.2:28332 \
               -zmqpubrawtx=ipc:///tmp/mundoteamd.tx.raw \
...
These options can also be provided in mundoteam.conf.
...
From the perspective of mundoteamd, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into mundoteamd directly. Furthermore, no information is
...
using. Mundoteamd appends an up-counting sequence number to each
