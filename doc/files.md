...
The data directory is the default location where the Mundoteam Core files are stored.

1. The default data directory paths for supported platforms are:

Platform | Data directory path
---------|--------------------
Linux    | `$HOME/.mundoteam/`
macOS    | `$HOME/Library/Application Support/Mundoteam/`
Windows  | `%APPDATA%\Mundoteam\` <sup>[\[1\]](#note1)</sup>

2. A custom data directory path can be specified with the `-datadir` option.

3. All content of the data directory, except for `mundoteam.conf` file, is chain-specific. This means the actual data directory paths for non-mainnet cases differ:
...
`blocks/`          | `blkNNNNN.dat`<sup>[\[2\]](#note2)</sup> | Actual Mundoteam blocks (in network format, dumped in raw on disk, 128 MiB per file)
...
`./`               | `mundoteam.conf`        | User-defined [configuration settings](bitcoin-conf.md) for `mundoteamd` or `mundoteam-qt`. File is not written to by the software and must be created manually. Path can be specified by `-conf` option
`./`               | `mundoteamd.pid`        | Stores the process ID (PID) of `mundoteamd` or `mundoteam-qt` while running; created at start and deleted on shutdown; can be specified by `-pid` option
`./`               | `debug.log`           | Contains debug information and general logging generated by `mundoteamd` or `mundoteam-qt`; can be specified by `-debuglogfile` option
...
`mundoteam-qt` uses [`QSettings`](https://doc.qt.io/qt-5/qsettings.html) class; this implies platform-specific [locations where application settings are stored](https://doc.qt.io/qt-5/qsettings.html#locations-where-application-settings-are-stored).

## Legacy subdirectories and files

These subdirectories and files are no longer used by the Mundoteam Core:
