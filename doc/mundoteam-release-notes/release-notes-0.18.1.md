Mundoteam Core version 0.18.1 is now available from:

  <https://download.mundoteam.org/mundoteam-0.18.1/>
...
`/Applications/Mundoteam-Qt` (on Mac) or `mundoteamd`/`mundoteam-qt` (on
Linux).
...
Mundoteam Core is supported and extensively tested on operating systems
using the Linux kernel, macOS 10.10+, and Windows 7 and newer. It is not
recommended to use Mundoteam Core on unsupported systems.
...
10.10. Additionally, Mundoteam Core does not yet change appearance when
...
- The `rpcallowip` option can no longer be used to automatically listen
  ...
  [...] -p 127.0.0.1:14473:14473` (this is an extra `:14473` over the
...
- The `whitelistforcerelay` option is used to relay transactions from
  ...
  [contacting](mailto:contact@mundoteam.org) the Mundoteam Core
  project to let us know about their use-case, as this feature could be
  deprecated in the future).

Wallet changes
--------------
When creating a transaction with a fee above `-maxtxfee` (default 0.1 MNT),
the RPC commands `walletcreatefundedpsbt` and  `fundrawtransaction` will now fail
instead of rounding down the fee. Beware that the `feeRate` argument is specified
in MNT per kilobyte, not litoshi per byte.
...
- A new [document](/blob/master/doc/mundoteam-conf.md)
  about the `mundoteam.conf` file describes how to use it to configure
  Mundoteam Core.

- A new document introduces Mundoteam Core's BIP174 [Partially-Signed
  Mundoteam Transactions
...
- A new `--disable-bip70` option may be passed to `./configure` to
  prevent Mundoteam-Qt from being built with support for the BIP70 payment
  protocol or from linking libssl.  As the payment protocol has exposed
  Mundoteam Core to libssl vulnerabilities in the past, builders who don't
...
- `getaddressinfo` now returns an additional `solvable` boolean field
  when Mundoteam Core knows enough about the address's scriptPubKey,
...
- `importprivkey` will preserve previously-set labels for addresses or
  ...
  wallet" in earlier releases of Mundoteam Core, subsequently importing
...
- `generate` is deprecated and will be fully removed in a subsequent
  ...
  in this version, restart mundoteamd with the `-deprecatedrpc=generate`
...
- A new `mundoteam-wallet` tool is now distributed alongside Mundoteam
...
This section describes planned changes to Mundoteam Core that may affect
other Mundoteam software and services.

- Since version 0.16.0, Mundoteam Core’s built-in wallet has defaulted to
  ...
  widely-used software.  Starting with Mundoteam Core 0.20 (expected about
  a year after 0.18), Mundoteam Core will default to native segwit
  ...
  to bech32 addresses, and if the Mundoteam Core project sees enough
  additional adoption, it will instead default to bech32 receiving
  addresses in Mundoteam Core 0.20.
  ...
  set the `addresstype=bech32` configuration option in any Mundoteam Core
...
- This release allows peers that your node automatically disconnected
  ...
  which you don't have many other peers).  Previously, Mundoteam Core
...
- A sub-project of Mundoteam Core now provides Hardware Wallet Interaction
  (HWI) scripts that allow command-line users to use several popular
  hardware key management devices with Mundoteam Core.  See their [project
...
- This release changes the Random Number Generator (RNG) used from
  OpenSSL to Mundoteam Core's own implementation, although entropy
  gathered by Mundoteam Core is fed out to OpenSSL and then read back in
  when the program needs strong randomness. This moves Mundoteam Core a
...
- On macOS, Mundoteam Core now opts out of application CPU throttling
...
- #14473 Let wallet `importmulti` RPC accept labels for standard scriptPubKeys (ryanofsky)
