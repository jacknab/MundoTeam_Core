...
const char * const BITCOIN_CONF_FILENAME = "mundoteam.conf";
...
#else
    const char* pszModule = "mundoteam";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Mundoteam";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/Mundoteam";
#else
    // Unix-like
    return pathRet / ".mundoteam";
#endif
#endif
}
